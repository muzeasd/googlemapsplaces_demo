...
//        mGeoDataClient = geoDataClient;
    ...
    private ArrayList<AutocompletePrediction> getAutocomplete(CharSequence constraint) {
        if(mGoogleApiClient.isConnected()){
            ...
            Log.i(TAG, "getAutocomplete2: Starting Autocomplete Query");
            ...
            // Confirm the query completed successfully
            final Status status = autocompletePredictions.getStatus();
            if(!status.isSuccess()){
                Toast.makeText(getContext(), "Error contacting API"+status.toString(), Toast.LENGTH_LONG).show();
                Log.e(TAG, "getAutocomplete2: "+ status.toString());
                autocompletePredictions.release();
                return null;
            }

            ArrayList<AutocompletePrediction> predictionArrayList = new ArrayList<>();
            for (AutocompletePrediction buffer : autocompletePredictions) {
                predictionArrayList.add(buffer);
            }
